function pcAnalysis(dataroot)

load(fullfile(dataroot, 'dbspont.mat'));
dex = 1;
...
for d = [1:length(db)]
    dat = load(fullfile(dataroot,sprintf('../spont_%s_%s.mat',db(d).mouse_name,db(d).date)));
    ...
    disp([results.runcorr(d) results.pupilcorr(d) results.whiskcorr(d)])
    ...
    disp([results.indist(:,d) results.outdist(:,d)]);
    
    disp([sum(bcells==1)/NN sum(bcells==2)/NN])
