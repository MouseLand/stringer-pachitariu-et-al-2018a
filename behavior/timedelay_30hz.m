...
for d = [1:length(dall.db)]
	...
	for td = 1%:length(tbins)
		...
		for l = 1:length(lams)
			...
			results.acBeh(:,:,td,l,d) = 0.5 * (acf(ytrain(1:16,:)', 1000) + acf(ytest(1:16,:)', 1000));
...
for d = 1:3
	for td = 1%:length(tbins)
		...
		acB(:,:,td,d) = results.acBeh(:,:,td,ilam,d);
	end
end
...
my_subplot(1,3,3);
plot([0:1000-1]*.03,squeeze(mean(acB,4)))
title('acB');
xlim([0 1]);
...
%plot([-1000:1000]*.03,crr / max(crr),'g')
ccSV = mean(results.ccSV(:,1,1,:),4);
plot([-1000:1000]*.03,ccSV,'r')
acm=acf(dat.beh.face.motionSVD(~logical(dat.stimtpt),1),1000);
acm = acm(1:end,:);
plot([1:1000]*.03+.03,acm ,'g');
ac=squeeze(mean(acB(1:end,1:3,:,:),4));
plot([1:1000]*.03, ac ,'m');
plot([-1000:1000]*.03, ccf(ytrain(1,:)',ytest(1,:)',1000)*-1,'c')
xlim([-3 3]);
legend('behavior->SVC1','neurons','behavior ACG','behav-neural ACG');
